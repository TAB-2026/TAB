#!/usr/bin/env python3
"""
Evaluation script for fine-tuned GPT-OSS-20B on TAB graph reasoning tasks.
...
Pipeline:
  ...
  4. Save per-task results to ``tasks/<task>/test_results_gpt_oss_20b_*.json``.

Run from the ``testing/`` directory: ``python test_gpt_oss_20b_finetuned.py``
"""
import os
import json
import torch
...
from simple_gpt_oss_20b_finetuning import GraphEnhancedLoRALLM
def load_finetuned_model(model_path: str = "../output/gpt_oss_20b_finetuned",
    ...
    if use_graph_bias:
        ...
        if os.path.exists(graph_bias_path):
            ...
            print(f"  Loading graph bias weights from {graph_bias_path}")
        else:
...
def generate_with_graph_bias(model, input_ids, attention_mask, graph_data, tokenizer, max_new_tokens=100,
    ...
    if hasattr(model, 'generate') and callable(getattr(model, 'generate', None)):
        try:
            ...
            print(f"Warning: Failed to use model's generate method, falling back to custom loop: {e}")
...
def extract_answer_from_generation(full_response: str, input_text_len: int) -> str:
    ...
    return answer_part
def test_model_on_tasks(model_path: str = "../output/gpt_oss_20b_finetuned",
                       base_model_name: str = "openai/gpt-oss-20b",
                       tasks_dir: str = "../tasks",
                       use_graph_bias: bool = True,
                       fast_generation: bool = False,
    ...
    print("=== Testing GPT-OSS-20B Fine-tuned Model ===")
    print(f"Model path: {model_path}")
    print(f"Base model: {base_model_name}")
    print(f"Use graph bias: {use_graph_bias}")
    print(f"Fast generation mode: {fast_generation}")
    if fast_generation:
        print("  ⚠️ Note: In fast generation mode, graph bias is not used during generation (speed prioritized)")
    ...
    if not task_dirs:
        print(f"❌ No test data files found in {tasks_dir}")
        return
    ...
    for task_name in task_dirs:
        ...
        try:
            ...
            print(f"❌ Error loading {test_json_path}: {e}")
            continue
        ...
        output_file = os.path.join(task_dir, f"test_results_gpt_oss_20b_{timestamp}.json")
        output_data = {
            'model_path': model_path,
...
def main():
    ...
    test_model_on_tasks(
        model_path="../output/gpt_oss_20b_finetuned",

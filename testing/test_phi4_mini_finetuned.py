#!/usr/bin/env python3
"""
Evaluation script for fine-tuned Phi-4-mini on TAB graph reasoning tasks.
...
Pipeline:
  ...
  4. Save per-task results to ``tasks/<task>/test_results_phi4_mini_*.json``.

Run from the ``testing/`` directory: ``python test_phi4_mini_finetuned.py``
"""
import os
import json
import torch
...
from simple_phi4_mini_finetuning import GraphEnhancedLoRALLM
def load_finetuned_model(model_path: str = "../output/phi4_mini_finetuned",
                        base_model_name: str = "microsoft/Phi-4-mini-instruct",
...
def extract_answer_from_generation(full_response: str, input_text_len: int) -> str:
    ...
    return answer_part
def test_model_on_tasks(model_path: str = "../output/phi4_mini_finetuned",
                       base_model_name: str = "microsoft/Phi-4-mini-instruct",
                       tasks_dir: str = "../tasks",
                       use_graph_bias: bool = True,
                       fast_generation: bool = False,
    ...
    print("=== === Testing Phi-4-mini-instruct Fine-tuned Model ===")
    ...
    for task_name in task_dirs:
        ...
        output_file = os.path.join(task_dir, f"test_results_phi4_mini_{timestamp}.json")
        output_data = {
            'model_path': model_path,
...
def main():
    ...
    test_model_on_tasks(
        model_path="../output/phi4_mini_finetuned",

#!/usr/bin/env python3
"""
LoRA fine-tuning script for GPT-OSS-20B with TAB graph attentional bias.
...
Also provides ``GraphEnhancedLLM`` (full fine-tune variant) and signal-based sample timeouts.
Default output: ``../output/gpt_oss_20b_finetuned``.
"""
...
class GraphBiasTrainer(Trainer):
    ...
    Overrides ``save_model`` so ``GraphEnhancedLoRALLM`` checkpoints include
  bias projection weights alongside LoRA adapters.
    """
    ...
    def save_model(self, output_dir=None, _internal_call=False):
        """Delegate checkpoint saving to ``GraphEnhancedLoRALLM.save_pretrained`` when applicable."""
        if isinstance(self.model, GraphEnhancedLoRALLM):
            ...
            if output_dir is None:
                output_dir = self.args.output_dir
            original_save_safetensors = getattr(self.args, 'save_safetensors', True)
...
class TimeoutError(Exception):
    """Raised when per-sample graph encoding exceeds the signal alarm timeout."""
    pass


def timeout_handler(signum, frame):
    """SIGALRM handler used to bound graph-encoding wall time per sample."""
...
class GraphEnhancedLoRALLM(nn.Module):
    ...
    def _create_node_token_mapping(self, input_ids):
        """Locate token indices of node identifiers (N1, [NODEID.X]) in the prompt."""
        ...
        if graph_start_idx == -1:
            ...
            if not node_tokens:
                print("⚠️ Warning：No node identifier was found（N1/N2/N3 or [NODEID.X]）")
            return node_tokens
        ...
        if not node_tokens:
            ...
            for node_text in all_node_texts[:20]:
                ...
                if not target_tokens or len(target_tokens) == 0:
                    try:
                        n_token = tokenizer.encode("N", add_special_tokens=False)
                        num_str = re.match(r'N(\d+)', node_text).group(1)
                        num_token = tokenizer.encode(num_str, add_special_tokens=False)
                        if n_token and num_token:
                            for i in range(len(input_ids_list) - len(n_token) - len(num_token) + 1):
                                if (input_ids_list[i:i+len(n_token)] == n_token and
                                    input_ids_list[i+len(n_token):i+len(n_token)+len(num_token)] == num_token):
        ...
        if not node_tokens:
            ...
            if graph_start_idx != -1:
                ...
                n_matches_in_graph = list(re.finditer(r'\bN(\d+)\b', graph_text))
                if n_matches_in_graph:
                    print(f"   Debug: Found {len(n_matches_in_graph)} node identifiers in graph sequence text (but unable to locate in token sequence)")
                    print(f"   Example node identifiers: {[m.group(0) for m in n_matches_in_graph[:5]]}")
                    if n_matches_in_graph:
                        sample_node = n_matches_in_graph[0].group(0)
                        sample_tokens = tokenizer.encode(sample_node, add_special_tokens=False)
                        print(f"   Example: '{sample_node}' encoded as tokens: {sample_tokens}")
                        if sample_tokens:
                            found = False
                            for i in range(len(input_ids_list) - len(sample_tokens) + 1):
                                if input_ids_list[i:i+len(sample_tokens)] == sample_tokens:
                                    print(f"   Found '{sample_node}' at position {i}")
                                    found = True
                                    break
                            if not found:
                                print(f"   Tokens for '{sample_node}' not found in input_ids")
                                print(f"   input_ids length: {len(input_ids_list)}")
                                print(f"   input_ids first 10: {input_ids_list[:10]}")
                                print(f"   input_ids last 10: {input_ids_list[-10:]}")
                                if len(sample_tokens) >= 2:
                                    first_token = sample_tokens[0]
                                    first_positions = [i for i, tok in enumerate(input_ids_list) if tok == first_token]
                                    if first_positions:
                                        print(f"   Found first token {first_token} at positions: {first_positions[:5]}")
                                        found_partial = False
                                        for pos in first_positions[:5]:
                                            if pos + 1 < len(input_ids_list) and input_ids_list[pos + 1] == sample_tokens[1]:
                                                print(f"   Found partial match at position {pos} (first two tokens)")
                                                found_partial = True
                                                if pos not in node_tokens:
                                                    node_tokens.append(pos)
                                        if not found_partial:
                                            print(f"   Warning: Sequence may be truncated, only found first token, second token not found")
                                            for pos in first_positions[:5]:
                                                if pos + 1 >= len(input_ids_list):
                                                    print(f"   Found first token at position {pos} (end of sequence), attempting to use this position")
                                                    if pos not in node_tokens:
                                                        node_tokens.append(pos)
                                                    break
                                                elif pos + 1 < len(input_ids_list):
                                                    next_token = input_ids_list[pos + 1]
                                                    print(f"   Next token at position {pos}: {next_token} (expected: {sample_tokens[1]})")
                                                    if next_token == 199999:
                                                        print(f"   Found first token at position {pos} (next is padding), attempting to use this position")
                                                        if pos not in node_tokens:
                                                            node_tokens.append(pos)
                                                        break
                                                    elif pos >= graph_start_token_idx:
                                                        print(f"   Found first token at position {pos} (in graph sequence part), attempting to use this position")
                                                        if pos not in node_tokens:
                                                            node_tokens.append(pos)
                                                        break
        return node_tokens
    ...
    def _compute_bias_regularization_loss(self, attn_bias):
        ...
        if attn_bias is not None and attn_bias.numel() > 0:
            ...
            l2_reg_loss = 1e-7 * (
                ...
                (F.softplus(self.graph_encoder.common_neighbors_weight) ** 2).sum() +
                (F.softplus(self.graph_encoder.clustering_coefficient_weight) ** 2).sum() +
                (F.softplus(self.graph_encoder.triangle_count_weight) ** 2).sum()
            )
    ...
    def _apply_graph_bias(self, outputs, attn_bias):
        ...
        for layer_idx, attention in enumerate(attentions):
            enhanced_attention = attention + self.bias_weight * attn_bias
            enhanced_attentions.append(enhanced_attention)
        outputs.attentions = enhanced_attentions
        return outputs
    ...
    def save_pretrained(self, save_directory, *args, **kwargs):
        ...
        graph_bias_state = {
            'bias_weight': self.bias_weight.data,
        }
    ...
    def __getattr__(self, name):
        ...
        if name in ['base_model', 'model']:
            raise AttributeError(f"'{self.__class__.__name__}' object has no attribute '{name}'. Use 'lora_model' instead.")
        try:
            return super().__getattr__(name)
        except AttributeError:
            return getattr(self.lora_model, name)
class GraphEnhancedLLM(nn.Module):
    """Full-model (non-LoRA) variant with graph bias injection."""

    def __init__(self, llm_model_name: str, graph_dim: int = 64, use_graph_bias: bool = True, debug: bool = False):
        super().__init__()
        ...
        if use_graph_bias:
            self.bias_projection = nn.Linear(
                self.graph_encoder.num_heads,
                self.llm.config.num_attention_heads
            )
    def forward(self, input_ids, attention_mask, graph_data):
        """Forward pass with optional post-hoc attention modification from graph bias."""
        attn_bias = None
        ...
        if attn_bias is not None:
            ...
            if hasattr(self, 'debug') and self.debug:
                print(f"Graph structure bias shape: {attn_bias.shape}")
                print(f"Node token positions: {node_token_indices}")
                print(f"Input text: {self.tokenizer.decode(input_ids, skip_special_tokens=True)}")
            if node_token_indices:
                outputs = self._apply_selective_graph_bias(outputs, attn_bias, node_token_indices)
            else:
                outputs = self._apply_graph_bias(outputs, attn_bias)
                print("Node tokens not found, using traditional method")
        return outputs

    def _apply_graph_bias(self, outputs, attn_bias):
        """Add graph bias uniformly to all attention head outputs (fallback when node mapping fails)."""
        attentions = outputs.attentions
        if attn_bias.shape[1] != self.llm.config.num_attention_heads:
            attn_bias = self.bias_projection(attn_bias.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        enhanced_attentions = []
        for layer_idx, attention in enumerate(attentions):
            enhanced_attention = attention + self.bias_weight * attn_bias
    ...
    def _create_node_token_mapping(self, input_ids):
        """Find token positions of [NODEID.X] markers in the input sequence."""
        node_tokens = []
        ...
        if not node_tokens:
            ...
            import re
            nodeid_matches = re.findall(r'\[NODEID\.([A-Z]+)\]', full_text)
            for node_id in nodeid_matches:
                node_text = f"[NODEID.{node_id}]"
                node_tokens.extend(self._find_token_positions(input_ids, node_text))
        return node_tokens

    def _find_token_positions(self, input_ids, target_text):
        """Return start indices where target_text token subsequence matches input_ids."""
        positions = []
    ...
    def _apply_selective_graph_bias(self, outputs, attn_bias, node_token_indices):
        ...
        if attn_bias.shape[-1] != num_nodes:
            if attn_bias.shape[-1] > num_nodes:
                ...
                attn_bias = self._expand_bias_matrix(attn_bias, num_nodes)
        for layer_idx, attention in enumerate(attentions):
            enhanced_attention = attention.clone()
            for i, node_i in enumerate(node_token_indices):
                for j, node_j in enumerate(node_token_indices):
                    if i < attn_bias.shape[-1] and j < attn_bias.shape[-1]:
                        bias_value = attn_bias[0, 0, i, j]
                        enhanced_attention[0, :, node_i, node_j] += self.bias_weight * bias_value
            attentions[layer_idx] = enhanced_attention
        return outputs
    def _expand_bias_matrix(self, attn_bias, target_size):
        """Zero-pad a square bias matrix up to target_size x target_size."""
        current_size = attn_bias.shape[-1]
    ...
    def generate(self, input_ids, attention_mask, graph_data, **kwargs):
        """Generate tokens via the base LLM (bias applied during forward pre-pass)."""
        with torch.no_grad():
            outputs = self.forward(input_ids, attention_mask, graph_data)
...
def convert_single_sample_with_timeout_and_bias(item: Dict[str, Any],
                                              graph_encoder: GraphSequenceEncoder,
    ...
    try:
        ...
        if graph_encoder.use_graph_bias and hasattr(graph_encoder, 'graphormer') and graph_encoder.graphormer is not None:
...
def convert_single_sample_with_timeout(item: Dict[str, Any],
                                     graph_encoder: GraphSequenceEncoder,
                                     timeout_seconds: int = 10) -> Dict[str, str]:
    """Convert one sample with SIGALRM timeout guard (Unix only)."""
    try:
        edge_index_data = item['edge_index']
        num_nodes = item['num_nodes']
        if isinstance(edge_index_data, list):
...
def convert_persistent_to_training_data_with_bias(persistent_data: List[Dict[str, Any]],
                                                graph_encoder: GraphSequenceEncoder,
    ...
    for i, item in enumerate(persistent_data):
        if i % 50 == 0:
            print(f"Processing progress: {i}/{len(persistent_data)} (success: {successful}, failed: {failed}, timeout: {timeout_count})")
        try:
            result = convert_single_sample_with_timeout_and_bias(item, graph_encoder, timeout_seconds)
            ...
            failed += 1
            continue
    print(f"Conversion completed: {successful} successful, {failed} failed, {timeout_count} timeout")
    return training_data


def convert_persistent_to_training_data(persistent_data: List[Dict[str, Any]],
                                      graph_encoder: GraphSequenceEncoder,
                                      timeout_seconds: int = 10) -> List[Dict[str, str]]:
    """Convert samples without graph bias (text-only graph sequences)."""
    training_data = []
    successful = 0
    failed = 0
    timeout_count = 0
    for i, item in enumerate(persistent_data):
        if i % 50 == 0:
            print(f"Processing progress: {i}/{len(persistent_data)} (success: {successful}, failed: {failed}, timeout: {timeout_count})")
        try:
            result = convert_single_sample_with_timeout(item, graph_encoder, timeout_seconds)
            if result is not None:
                training_data.append(result)
                successful += 1
            else:
...
def train_enhanced_gpt_oss_20b(use_tasks_dataset: bool = True,
                       tasks_dir: str = "../tasks",
                       data_dir: str = "./graph_qa_data",
                       max_samples: int = 1000,
                       use_graph_bias: bool = True):
    """GPT-OSS-20B with graph bias and trainable bias projection (TAB enhanced mode)."""
    print("=== Enhanced GPT-OSS-20B Fine-tuning (LoRA fine-tuning with graph structure bias) ===")
    ...
    if not tasks_data:
        ...
        return None, None
    training_data = convert_persistent_to_training_data_with_bias(tasks_data, graph_encoder, timeout_seconds=10)
    print("\n2. Creating dataset...")
    ...
    print(f"Successfully created training dataset with {len(dataset)} samples")
    output_dir = '../output/gpt_oss_20b_finetuned'
    ...
    if use_graph_bias:
        print("Note: LoRA adapter weights and graph bias weights have been saved, PEFT library is required for loading")
    else:
        print("Note: LoRA adapter weights have been saved, PEFT library is required for loading")
    return model, tokenizer
def train_simple_gpt_oss_20b(use_tasks_dataset: bool = True,
                     tasks_dir: str = "../tasks",
                     data_dir: str = "./graph_qa_data",
                     max_samples: int = 1000):
    """GPT-OSS-20B with LoRA only (no graph bias wrapper)."""
    print("=== Simple GPT-OSS-20B Fine-tuning (LoRA fine-tuning) ===")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model_name = "./gpt-oss-20b-hf"
    hf_model_name = "openai/gpt-oss-20b"
    if os.path.exists(model_name) and os.path.isdir(model_name):
    ...
    if not tasks_data:
        print("❌ Failed to load any tasks data")
        return None, None
    training_data = convert_persistent_to_training_data(tasks_data, graph_encoder, timeout_seconds=10)
    print("\n2. Creating dataset...")
    dataset = SimpleGraphDataset(training_data, tokenizer, max_length=512)
    print(f"Successfully created training dataset with {len(dataset)} samples")
    output_dir = '../output/simple_gpt_oss_20b_finetuned'
    os.makedirs(output_dir, exist_ok=True)
    training_args = TrainingArguments(
        output_dir=output_dir,
        num_train_epochs=10,
    ...
    return model, tokenizer
def main():
    """Entry point: run enhanced GPT-OSS-20B fine-tuning with graph bias enabled."""
    model, tokenizer = train_enhanced_gpt_oss_20b(

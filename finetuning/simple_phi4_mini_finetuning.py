#!/usr/bin/env python3
"""
LoRA fine-tuning script for Phi-4-mini with TAB graph attentional bias.
...
Default output: ``../output/phi4_mini_finetuned`` (enhanced) or ``../output/simple_phi4_mini_finetuned``.
"""
...
def train_enhanced_phi4_mini(
    ...
    """Phi-4-mini with graph bias and trainable bias projection (TAB enhanced mode)."""
    print("=== Enhanced Phi-4-mini-instruct Fine-tuning (LoRA fine-tuning with graph structure bias) ===")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model_name = "./Phi-4-mini-instruct-hf"
    hf_model_name = "microsoft/Phi-4-mini-instruct"
    ...
    output_dir = "../output/phi4_mini_finetuned"
...
def train_simple_phi4_mini(
    use_tasks_dataset: bool = True,
    tasks_dir: str = "../tasks",
    data_dir: str = "./graph_qa_data",
    max_samples: int = 1000,
):
    """Phi-4-mini with LoRA only (no graph bias wrapper)."""
    print("=== Simple Phi-4-mini-instruct Fine-tuning (LoRA fine-tuning) ===")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model_name = "./Phi-4-mini-instruct-hf"
    hf_model_name = "microsoft/Phi-4-mini-instruct"
    if os.path.exists(model_name) and os.path.isdir(model_name):
    ...
    output_dir = "../output/simple_phi4_mini_finetuned"
    os.makedirs(output_dir, exist_ok=True)
    training_args = TrainingArguments(
        output_dir=output_dir,
        num_train_epochs=10,
    ...
    return model, tokenizer
def main():
    """Entry point: run enhanced Phi-4-mini fine-tuning with graph bias enabled."""
    model, tokenizer = train_enhanced_phi4_mini(
    ...
    if model is not None and tokenizer is not None:
        print("✅ Phi-4-mini-instruct finetuning completed successfully")
    else:
        print("❌ Phi-4-mini-instruct finetuning failed")

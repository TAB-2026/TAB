#!/usr/bin/env python3
"""
LoRA fine-tuning script for Llama-2-7B with TAB graph attentional bias.
...
Default output: ``../output/llama2_7b_finetuned`` (enhanced) or ``../output/simple_llama2_7b_finetuned``.
"""
    ...
    def _create_node_token_mapping(self, input_ids):
        ...
        if graph_start_idx == -1:
            ...
            if not node_tokens:
                ...
                print(f"   'Graph:' marker not found, searched entire text")
...
def train_enhanced_llama2_7b(
    ...
    """Llama-2-7B with graph bias and trainable bias projection (TAB enhanced mode)."""
    print("=== Enhanced Llama-2-7B Fine-tuning (LoRA fine-tuning with graph structure bias) ===")
    ...
    output_dir = "../output/llama2_7b_finetuned"
...
def train_simple_llama2_7b(
    use_tasks_dataset: bool = True,
    tasks_dir: str = "../tasks",
    data_dir: str = "./graph_qa_data",
    max_samples: int = 1000,
):
    """Llama-2-7B with LoRA only (no graph bias wrapper)."""
    print("=== Simple Llama-2-7B Fine-tuning (LoRA fine-tuning) ===")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model_name = "./Llama-2-7b-hf"
    hf_model_name = "meta-llama/Llama-2-7b-hf"
    if not os.path.exists(model_name):
    ...
    output_dir = "../output/simple_llama2_7b_finetuned"
    os.makedirs(output_dir, exist_ok=True)
    training_args = TrainingArguments(
        output_dir=output_dir,
        num_train_epochs=10,
    ...
    return model, tokenizer
def main():
    """Entry point: run enhanced Llama-2-7B fine-tuning with graph bias enabled."""
    model, tokenizer = train_enhanced_llama2_7b(
    ...
    if model is not None and tokenizer is not None:
        print("✅ Llama-2-7B enhanced training completed successfully")
    else:
        print("❌ Llama-2-7B enhanced training failed")
